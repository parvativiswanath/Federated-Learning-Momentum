...
def upload_file():
    ...
    with lock:
        ...
        if len(client_models) >= num_clients:  
            ...
            global_velocity = fedAdan(global_model, client_models, client_velocities)
...
def fedAdan(global_model, clientModels, client_velocities):
    ...
    with torch.no_grad():
        # Calculate the average
        for param_name in model_state_dict:
            client_params = [client_model[param_name] for client_model in clientModels]
            averaged_param = torch.stack(client_params).mean(dim=0)
